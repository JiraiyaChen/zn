...
const content = `<template>
  ...
  <Teleport to="body">
    <div v-if="isFullscreen" class="cd-fullscreen-overlay" @click.self="toggleFullscreen">
      <div class="cd-fs-task">{{ runningTaskName || '专注倒计时' }}</div>
      <div class="cd-fs-clock" :class="{ urgent: cdRemaining <= 10 && cdRemaining > 0 }">
        {{ cdDisplayText }}
      </div>
      <div class="cd-fs-progress-track">
        <div class="cd-fs-progress-fill" :style="{ width: progressPct + '%' }"></div>
      </div>
      <div v-if="showDone" class="cd-fs-done">✦ 任务完成，好好休息 ✦</div>
      <div class="cd-fs-hint">点击空白处退出全屏</div>
    </div>
  </Teleport>
  ...
  <div class="countdown-section">
    ...
    <input class="cd-task-input" v-model="taskName" type="text" placeholder="这段时间，你想做什么？" maxlength="40" />
    ...
    <div class="cd-presets">
      <button
        ...
        :key="mins"
        class="cd-preset-btn"
        :class="{ active: !cdRunning && cdH === 0 && cdS === 0 && cdM === mins }"
    ...
    <div class="cd-controls">
      ...
      <button class="cd-btn primary" @click="toggleCountdown">{{ cdRunning ? '⏸' : '▶' }}</button>
      <button class="cd-btn" @click="toggleFullscreen" :title="isFullscreen ? '退出全屏' : '全屏'">
        <span class="cd-fs-icon">{{ isFullscreen ? '↙' : '↗' }}</span>
      </button>
    </div>

    <div class="cd-history">
      <div class="cd-history-title">已完成任务</div>
      <div v-if="!cdHistory.length" class="cd-history-empty">完成第一个任务后，它会出现在这里。</div>
      <div v-for="(item, i) in cdHistory" :key="i" class="cd-history-item">
        <span class="cd-history-name">{{ item.name }}</span>
        <span class="cd-history-meta">{{ item.duration }} &nbsp;·&nbsp; {{ item.time }}</span>
      </div>
    </div>
  </div>
</template>
...
function onVisibilityChange() {
  if (document.visibilityState === 'visible' && cdRunning.value) acquireWakeLock();
}
...
function cdQuickSet(mins) {
  ...
  cdH.value = 0; cdM.value = mins; cdS.value = 0;
}
...
try {
  cdHistory.value = JSON.parse(localStorage.getItem('mindful_cd_history') || '[]');
} catch {
  cdHistory.value = [];
}
